...
test('Customer Login Flow Testing ', async ({ page }) => {
    ...
    await page
        ...
        .fill(signinData.password);
    await page.getByRole('button', { name: 'Sign in' }).click();
    await expect(
        page.getByRole('heading', { name: 'Grab & Go' }),
    ).toBeVisible();
});
...
test('Customer Signup Flow Testing', async ({ page }) => {
    ...
    await expect(
        page.getByRole('heading', { name: "Let's Create Your Account" }),
    ).toBeVisible();
    ...
    await page
        ...
        .fill(createAccountData.email);
    await page
        .getByLabel('Password', { exact: true })
        .fill(createAccountData.password);
    await page
        .getByRole('textbox', { name: 'Enter your password' })
        .fill(createAccountData.password);
    await page.getByRole('button', { name: 'Continue', exact: true }).click();
    await expect(
        page.getByRole('heading', { name: 'Last Step' }),
    ).toBeVisible();

    await page
        .getByPlaceholder('Enter your name...')
        .fill(createAccountData.name);
    await page.getByLabel('Address').fill(createAccountData.address);
    await page
        .getByRole('option', {
            name: '1805Sixth Avenue New Westminster, BC, Canada',
        })
        .click();
